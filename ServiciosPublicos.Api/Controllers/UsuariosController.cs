    ...
    public class UsuariosController : BaseApiController
    {
        ...
        public async Task<HttpResponseMessage> forgotPassword(HttpRequestMessage request, string email)
        {
            return await CreateHttpResponseAsync(request, async () => {
                ...
                try {
                   List<string> token = _usuarioservice.forgotPassword(email, out message);
        ...
        public async Task<HttpResponseMessage> verificarToken(HttpRequestMessage request, string token, string tokenA, string fechaToken, string email)
        {
            return await CreateHttpResponseAsync(request, async () => {
                HttpResponseMessage response = null;
                string message = String.Empty;
                ...
                try
                {
                    bool cambiarPassword = _usuarioservice.verificarToken(token, tokenA, fechaToken, email);
        ...
        public async Task<HttpResponseMessage> modificarPassword(HttpRequestMessage request, string idUsuario , string password)
        {
            return await CreateHttpResponseAsync(request, async () => {
                HttpResponseMessage response = null;
                string message = String.Empty;
                ...
                try
                {
                    _usuarioservice.modificarPassword(idUsuario, password, out message);
